...
func main() {
   ...
   aDb.group["g1"] = &tGroup{Uid: map[string]*tMember{
      "u111111": &tMember{Alias: tAlias{En: "hello",Nat:"1234"}, Status:eStatJoined},
      "u222222": &tMember{Alias: tAlias{En: "hello22"}, Status:eStatJoined},
      "u333333": &tMember{Alias: tAlias{En: "hello22"}, Status:eStatJoined},
   }}
...
type tUserDb struct {
   ...
   // change to map[string]tAlias/*tAlias?
...
type tUser struct {
   door sync.RWMutex
   Nodes map[string]tNode // key is NodeId, value is tNode
...
func (o *tUserDb) Test(iUid string, iGid string) error {
  ...
  aUser.door.Lock()
  aUser.Nodes = map[string]int{"node1": 0}
  aUser.Aliases = []tAlias { {En: "patricia", Nat: "パト"}}
  aUser.door.Unlock()
  ...
  aGroup.door.Lock()
  aGroup.Uid = map[string]*tMember{iUid: o.user[iUid]}
  aGroup.door.Unlock()
  ...
  o.groupDoor.Unlock()
   ...
   o.user[iUid] = &tUser{}
   o.user[iUid].Aliases = []tAlias { {En: "patricia", Nat: "パト"}}
   fmt.Println()
   fmt.Println("************************************************")
   fmt.Printf("the uid is %v, alias %v \n", iUid, o.user[iUid].Aliases[0].En)
   fmt.Println()

   o.user[iUid].Nodes =map[string]int{"t":0, "c":1}

   o.group[iGid] = &tGroup{}
   o.group[iGid].Uid = map[string]*tMember{}
   o.group[iGid].Uid[iUid] = &tMember{}
   aPatriciaAlias := tAlias {En: "patricia", Nat: "パト"}
   o.group[iGid].Uid[iUid].Alias = aPatriciaAlias
   o.group[iGid].Uid[iUid].Status = eStatJoined

   o.group[iGid] = &tGroup{Uid: map[string]*tMember{}} // definition of group
   aMember := &tMember{Alias: tAlias{En: "sam"}, Status: eStatJoined} // definition of member
   o.group[iGid].Uid[iUid] = aMember // assignment
...
//: below is the public api
  ...
  func (o *tUserDb) AddUser(iUid, iNewNode string, iAliases []string) (aAliases []string, err error) {
     ...
     /*
     Check if iUid is in db. If it is, return error.
     If iUid is not in db, check disk. If it is, return error.

     Create a new tUser with that iUid.
